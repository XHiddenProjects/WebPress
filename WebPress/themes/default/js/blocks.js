...
function dropItem(ev) {
  ...
  switch(data){
	  ...
	  let head = prompt('Include Heading; Yes/No');
	...
	for(let j=0;j<d.length;j++){
				if(d[j].checked){
					...
					switch(val){
						...
						switch(pos){
							case 'top':
							const tc = elm.parentElement.querySelector('input[type="color"]').value;
							const ts = elm.value + elm.parentElement.querySelector('select.bsize').value;
							const tss = elm.parentElement.querySelector('select.bstyle').value;
							targetArea.style.borderTop = ts+' '+tss+' '+tc;
							break;
								case 'right':
							 const rc = elm.parentElement.querySelector('input[type="color"]').value;
							 const rs = elm.value + elm.parentElement.querySelector('select.bsize').value;
							 const rss = elm.parentElement.querySelector('select.bstyle').value;
							targetArea.style.borderRight = rs+' '+rss+' '+rc;
							break;
								case 'bottom':
							 const bc = elm.parentElement.querySelector('input[type="color"]').value;
							 const bs = elm.value + elm.parentElement.querySelector('select.bsize').value;
							 const bss = elm.parentElement.querySelector('select.bstyle').value;
							targetArea.style.borderBottom = bs+' '+bss+' '+bc;
							break;
								case 'left':
							 const lc = elm.parentElement.querySelector('input[type="color"]').value;
							 const ls = elm.value + elm.parentElement.querySelector('select.bsize').value;
							 const lss = elm.parentElement.querySelector('select.bstyle').value;
...
function makeShadowBox(elem){
	...
	for(let j=0;j<d.length;j++){
				if(d[j].checked){
					...
					if(!no.checked){
					const h = elem.parentElement.querySelector('.hshadow').value+elem.parentElement.querySelector('.hshadow').parentElement.querySelector('select').value;
					const v = elem.parentElement.querySelector('.vshadow').value+elem.parentElement.querySelector('.vshadow').parentElement.querySelector('select').value;
					const b = elem.parentElement.querySelector('.blur').value+elem.parentElement.querySelector('.blur').parentElement.querySelector('select').value;
					const s = elem.parentElement.querySelector('.spread').value+elem.parentElement.querySelector('.spread').parentElement.querySelector('select').value;
					const c = elem.parentElement.querySelector('.color').value;
					const i = (elem.parentElement.querySelector('.sinset').checked ? 'inset' : '');
...
function makeAnimate(ani){
	...
	for(let j=0;j<d.length;j++){
				if(d[j].checked){
					...
					switch(val){
					case 'target':
					const animate = ani.value;
					const timing = ani.parentElement.querySelector('.atime').value;
					const direction = ani.parentElement.querySelector('.adir').value;
					const fill = ani.parentElement.querySelector('.afill').value;
					const duration = ani.parentElement.querySelector('.adur').value + ani.parentElement.querySelector('.adur').parentElement.querySelector('select').value;
					const delay = ani.parentElement.querySelector('.adel').value + ani.parentElement.querySelector('.adel').parentElement.querySelector('select').value;
					const count = (ani.parentElement.querySelector('.acount').value<0 ? 'infinite' : ani.parentElement.querySelector('.acount'));
...
function makeWidth(v){
	...
	for(let j=0;j<d.length;j++){
				if(d[j].checked){
					...
					switch(val){
					case 'target':
						const width = (v.value<0 ? 'auto' : v.value + v.parentElement.querySelector('select').value);
...
function makeHeight(v){
	...
	for(let j=0;j<d.length;j++){
				if(d[j].checked){
					...
					switch(val){
					case 'target':
						const height = (v.value<0 ? 'auto' : v.value + v.parentElement.querySelector('select').value);
	...
	for(let j=0;j<d.length;j++){
				if(d[j].checked){
					...
					case 'target':
						switch(pos){
							case 'top':
								const tops = (v.value<0 ? 'auto' : v.value+v.parentElement.querySelector('select').value);
								targetArea.style.top = tops;
							break;
							case 'right':
								const right = (v.value<0 ? 'auto' : v.value+v.parentElement.querySelector('select').value);
								targetArea.style.right = right;
							break;
							case 'bottom':
								const bottom = (v.value<0 ? 'auto' : v.value+v.parentElement.querySelector('select').value);
								targetArea.style.bottom = bottom;
							break;
							case 'left':
								const left = (v.value<0 ? 'auto' : v.value+v.parentElement.querySelector('select').value);
...
function makeTransform(v){
	...
	for(let j=0;j<d.length;j++){
				if(d[j].checked){
					...
					switch(val){
					case 'target':
					const set = (v.parentElement.querySelector('select').value==='none'? 'none' : v.parentElement.querySelector('select').value.toLowerCase()+'('+v.parentElement.querySelector('input').value+')');
