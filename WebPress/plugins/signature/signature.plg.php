	...
	function signature_afterMsg(){
		...
		if($d['active']){
			if(array_key_exists($author, $d['config']['valid']))
				$out.='<blockquote class="mt-4 mb-3 signatureBox" style="border-left-color:#07d5df; border-right-color:#07d5df">'.html_entity_decode($d['config']['valid'][$author], ENT_QUOTES).'</blockquote>';
		}
		return $out;
	}
	?>