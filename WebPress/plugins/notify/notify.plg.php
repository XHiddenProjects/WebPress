...
function notify_alert($name, $type, $msg, $active='on'){
	$out = '';
	if($active){
	$out.='<div name="'.$name.'" class="m-0 alert-dismissible fade show alert alert-'.$type.'" role="alert">'.$msg.'<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
	}
	return $out;
}
?>