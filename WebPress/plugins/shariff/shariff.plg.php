...
function shariff_bottomTopic()
{
  ...
  if ($data['active'])
  {
	$topicEntry = WebDB::getDB('topics', $topicID);
	$Url = (isset($_SERVER['HTTPS'])&&$_SERVER['HTTPS']==='on' ? 'https://' : 'http://') . $data['config'][$plugin.'domaine'] .'/' . MAINDIR. DS. 'forum.php' . DS . 'view?id=' . $topicID;
