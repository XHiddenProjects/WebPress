...
<input class="form-control" type="text" maxlength="6" id="captcha" name="captcha" required="true" aria-describedby="captcha"/>
  <span class="input-group-text" id="captcha"><img src="'.$BASEPATH.'/libs/captcha/img.php?captcha=w-'.$w.'+h-'.$h.'+bgr-'.$bgr.'+bgg-'.$bgg.'+bgb-'.$bgb.'+cr-'.$cr.'+cg-'.$cg.'+cb-'.$cb.'+size-'.$size.'+angle-'.$angle.'" id="captchaImg"/><button type="button" class="btn btn-secondary ms-1 captcha-reload" style="font-size:25px;"><i class="fa-solid fa-arrows-rotate"></i></button></span>
</div>';
		$out.='<script>
