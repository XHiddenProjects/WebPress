	...
	if(isset($_POST['saveFile'])){
		...
		<thead class="position-sticky top-0">
		 <tr>
		  ...
		  <th scope="col">'.$lang['events.device'].'</th>
		  <th scope="col">'.$lang['events.browser'].'</th>
		  <th scope="col">'.$lang['events.loc'].'</th>
	  ...
	  <tbody>
	   ...
	   foreach($expoit as $item){
		   ...
		   for($i=0;$i<count($setItem);$i++){
			   if(isset($_GET['filter'])&&isset($_GET['val'])){
				   if($_GET['filter']==='status'){
					   if(in_array($_GET['val'],$setItem)){
						   $out.='<td>'.$setItem[$i].'</td>'; 
					   }
				   }
				   if($_GET['filter']==='target'){
					   if(in_array($_GET['val'],$setItem)){
						   $out.='<td>'.$setItem[$i].'</td>'; 
					   }
				   }
	...
	if(isset($_POST['viewEvents'])){
		if(CSRF::check()!=='tokenExpired'&&CSRF::check()!=='invalidKey'){
			...
			echo '<script>if(window.location.hash!=="load"){
					window.location.open("./events#load");
			}</script>';
		}else{
